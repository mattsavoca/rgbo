...
def create_calculator_tab(processed_data_state: gr.State): # <<< Accept shared state
    ...
    with gr.Blocks() as calculator_tab:
        ...
        # --- ADDED: Zip Upload Section ---
        with gr.Accordion("Upload Unit Data from Zip File", open=False):
            gr.Markdown(
                "Upload a zip file containing unit CSV data (e.g., 'apt_UNITNAME.csv' files generated by the DHCR Parser). "
                "This will load the data for preview and allow combining into XLSX."
            )
            ...
            process_zip_button = gr.Button("Load Data from Zip")
            zip_status_output = gr.Textbox(label="Zip Processing Status", interactive=False, lines=2)
        # --- END Zip Upload Section ---
        ...
        # --- ADDED: Function and Handler for Zip Upload ---
        def process_uploaded_zip(zip_filepath_obj: Optional[gr.utils.NamedString]):
            """
            Processes an uploaded zip file containing unit CSVs.
            Unzips, reads CSVs into Polars DataFrames, and updates the shared state.
            """
            if zip_filepath_obj is None:
                return "Error: No zip file provided.", {} # Return empty dict for state

            zip_filepath = Path(zip_filepath_obj.name) # Get the actual path string
            log.info(f"Processing uploaded zip file: {zip_filepath}")

            if not zip_filepath.exists() or not zip_filepath.is_file():
                return f"Error: Zip file path is invalid or does not exist: {zip_filepath}", {}

            if zip_filepath.suffix.lower() != ".zip":
                 return "Error: Uploaded file is not a zip file.", {}

            loaded_data: Dict[str, pl.DataFrame] = {}
            status_messages = [f"Processing {zip_filepath.name}..."]

            try:
                with tempfile.TemporaryDirectory(prefix="calc_zip_") as temp_dir_str:
                    temp_dir = Path(temp_dir_str)
                    log.info(f"Extracting zip contents to temporary directory: {temp_dir}")
                    
                    try:
                        with zipfile.ZipFile(zip_filepath, 'r') as zip_ref:
                            # Check for potentially malicious paths (e.g., absolute paths, path traversal)
                            for member in zip_ref.infolist():
                                member_path = Path(member.filename)
                                if member_path.is_absolute() or '..' in member_path.parts:
                                    raise ValueError(f"Zip file contains potentially unsafe path: {member.filename}")
                            # Extract safely
                            zip_ref.extractall(temp_dir)
                        status_messages.append("Zip file extracted successfully.")
                    except zipfile.BadZipFile:
                         log.error(f"Invalid or corrupted zip file: {zip_filepath.name}")
                         return f"Error: Invalid or corrupted zip file: {zip_filepath.name}", {}
                    except ValueError as ve:
                        log.error(f"Error during zip extraction (unsafe path?): {ve}")
                        return f"Error: Zip extraction failed - {ve}", {}
                    except Exception as e:
                        log.error(f"Error extracting zip file {zip_filepath.name}: {e}", exc_info=True)
                        return f"Error extracting zip file: {e}", {}

                    # Scan for 'apt_*.csv' files in the temporary directory
                    csv_files = list(temp_dir.glob('**/apt_*.csv')) # Search recursively
                    if not csv_files:
                        status_messages.append("No 'apt_*.csv' files found in the zip archive.")
                        log.warning(f"No 'apt_*.csv' files found in extracted content from {zip_filepath.name}")
                        # Keep existing state if no new files found
                        # Need to access current state? No, this function *returns* the new state.
                        # If no CSVs, return empty dict to effectively clear or replace state.
                        return "\n".join(status_messages), {} 

                    status_messages.append(f"Found {len(csv_files)} potential unit CSV files. Reading data...")
                    
                    for csv_file in csv_files:
                        # Extract unit name from filename (e.g., apt_1A.csv -> 1A)
                        match = re.match(r"apt_(.+)\.csv", csv_file.name, re.IGNORECASE)
                        if match:
                            unit_name = match.group(1).replace('_', '/') # Sanitize/normalize if needed
                            try:
                                log.info(f"Reading CSV: {csv_file.name} for unit '{unit_name}'")
                                # try_parse_dates=True is often useful
                                df = pl.read_csv(csv_file, try_parse_dates=True) 
                                if not df.is_empty():
                                    loaded_data[unit_name] = df
                                    log.info(f"Successfully loaded data for unit '{unit_name}'. Shape: {df.shape}")
                                else:
                                    log.warning(f"CSV file '{csv_file.name}' for unit '{unit_name}' is empty. Skipping.")
                            except Exception as e:
                                log.error(f"Error reading CSV file {csv_file.name} for unit '{unit_name}': {e}", exc_info=True)
                                status_messages.append(f"Warning: Failed to read CSV '{csv_file.name}': {e}")
                        else:
                             log.warning(f"Skipping file - does not match 'apt_*.csv' pattern: {csv_file.name}")

                # Temporary directory is automatically cleaned up here
                status_messages.append(f"Successfully loaded data for {len(loaded_data)} units.")
                log.info(f"Finished processing zip. Loaded {len(loaded_data)} units.")
                return "\n".join(status_messages), loaded_data

            except Exception as e:
                log.error(f"An unexpected error occurred processing zip {zip_filepath.name}: {e}", exc_info=True)
                return f"Error: An unexpected error occurred: {e}", {}

        process_zip_button.click(
            fn=process_uploaded_zip,
            inputs=[zip_file_input],
            outputs=[zip_status_output, processed_data_state] # Update status and the shared state
        )
        # --- END Zip Upload Handler ---
