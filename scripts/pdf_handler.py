...
def run_pdf_processing(pdf_path: Path, base_output_dir: Path, generate_images: bool, calculate_vacancy: bool) -> Tuple[List[Dict[str, object]], Optional[Path]]:
    """
    ...
    Returns:
        A tuple containing:
        - A list of dictionaries, each representing an apartment unit with paths to its files.
          Paths are relative to base_output_dir.
    ...
    # Use the PDF's stem (filename without extension) as the subdirectory name
    run_output_dir_name = pdf_path.stem
    run_output_dir = base_output_dir / run_output_dir_name
    run_output_dir.mkdir(parents=True, exist_ok=True)
    logging.info(f"Created output directory for this run: {run_output_dir}")

    try:
        # Ensure the environment (like API keys loaded via dotenv in pdf_pipeline) is ready
        logging.info(f"Starting PDF processing for {pdf_path}...")
        process_pdf(pdf_path, run_output_dir, generate_images=generate_images)
        logging.info(f"PDF processing finished for {pdf_path}.")

    except Exception as e:
        logging.error(f"Error during pdf_pipeline.process_pdf execution: {e}", exc_info=True)
        # Return empty results but provide the directory path for potential cleanup
        run_output_dir_name = pdf_path.stem # Define it here too for safety
        return [], Path(run_output_dir_name) # Return relative path

    # --- Calculate Vacancy Allowances ---
    run_output_dir_name = pdf_path.stem # Ensure this is defined before use
    run_output_dir = base_output_dir / run_output_dir_name # Reconstruct full path if needed

    if calculate_vacancy:
        logging.info("Vacancy calculation requested.")
        if RGBO_DATA is None or RGBO_DATA.is_empty():
            logging.warning("Skipping vacancy allowance calculation as RGBO data is unavailable.")
        else:
            logging.info("Calculating vacancy allowances for generated unit CSVs...")
            for csv_file in run_output_dir.glob('apt_*.csv'):
                logging.info(f"Processing vacancy allowance for: {csv_file.name}")
                try:
                    # Load CSV
         ...
         logging.info("Vacancy calculation was not requested. Skipping.")

    # --- Scan for results ---
    results = []
    apt_files: Dict[str, Dict[str, object]] = {}
    image_dir: Optional[Path] = None
    if generate_images:
        image_dir = run_output_dir / f"{pdf_path.stem}_images" # run_output_dir should be defined now

    # Scan for CSV files and try to extract unit name
    for csv_file in run_output_dir.glob('apt_*.csv'):
        match = re.match(r"apt_(.+)\.csv", csv_file.name)
        unit_name = match.group(1).replace('_', '/') if match else csv_file.stem # Fallback, replace _ with /
        relative_csv_path = Path(run_output_dir_name) / csv_file.name
        apt_files[unit_name] = {
            'unit_name': unit_name,
            'csv_path': relative_csv_path,
            'img_paths': [] # Initialize image paths list
        }
        logging.debug(f"Found CSV for unit '{unit_name}': {relative_csv_path}")

    # Scan for image files if generated
    if image_dir and image_dir.exists():
        for img_file in image_dir.iterdir():
            if img_file.is_file() and img_file.suffix.lower() in ['.png', '.jpg', '.jpeg']:
                relative_img_path = Path(run_output_dir_name) / image_dir.name / img_file.name
                logging.debug(f"Found image: {relative_img_path}")
                # Add image path to all units found for this PDF run
                for unit_data in apt_files.values():
                     # Avoid duplicates if script re-runs logic somehow
                    if relative_img_path not in unit_data['img_paths']:
                        unit_data['img_paths'].append(relative_img_path)

    results = list(apt_files.values())

    if not results:
        logging.warning(f"No apartment CSV files found in {run_output_dir} after processing. Check PDF content and processing logs.")

    # Return the list of results and the relative path to the run directory
    return results, Path(run_output_dir_name)
